                    ...
                    if( func->n_args == 2 ) {
                        ...
                        if( !not_matching && ((left_arg_type2->type == left_type2->type ||
                            ...
                            if (i < n_interface_proc) {
                                ...
                                // order matches the function's formal parameters.
                                left_call_arg.loc = left->base.loc, left_call_arg.m_value = left;
                                a_args.push_back(al, left_call_arg);
                                right_call_arg.loc = right->base.loc, right_call_arg.m_value = right;
                                a_args.push_back(al, right_call_arg);
...
ASR::asr_t* ExprStmtWithScopeDuplicator::duplicate_DoConcurrentLoop(ASR::DoConcurrentLoop_t* x) {
    SymbolTable* parent_scope = current_scope;
    SymbolTable* loop_scope = al.make_new<SymbolTable>(parent_scope);

    SymbolDuplicator symbol_duplicator(al);
    symbol_duplicator.duplicate_SymbolTable(x->m_symtab, loop_scope);

    current_scope = loop_scope;

    Vec<ASR::do_loop_head_t> heads;
    heads.reserve(al, x->n_head);
    ...
    Vec<ASR::expr_t*> shared;
    shared.reserve(al, x->n_shared);

    for (size_t i = 0; i < x->n_shared; i++) {
        shared.push_back(al, duplicate_expr(x->m_shared[i]));
    }

    Vec<ASR::expr_t*> local;
    local.reserve(al, x->n_local);

    for (size_t i = 0; i < x->n_local; i++) {
        local.push_back(al, duplicate_expr(x->m_local[i]));
    }

    Vec<ASR::reduction_expr_t> reductions;
    reductions.reserve(al, x->n_reduction);

    for (size_t i = 0; i < x->n_reduction; i++) {
        ASR::reduction_expr_t reduction;
        reduction.loc = x->m_reduction[i].loc;
        reduction.m_op = x->m_reduction[i].m_op;
        reduction.m_arg = duplicate_expr(x->m_reduction[i].m_arg );
        reductions.push_back(al, reduction);
    }
    ...
    for (size_t i = 0; i < x->n_body; i++) {
        body.push_back(al, duplicate_stmt(x->m_body[i]));
    }

    ASR::asr_t* duplicated_loop = ASR::make_DoConcurrentLoop_t(al, x->base.base.loc, loop_scope,
                                                                heads.p, heads.n, shared.p,
                                                                shared.n, local.p, local.n,
                                                                reductions.p, reductions.n,
                                                                body.p, body.n);

    current_scope = parent_scope;
    return duplicated_loop;
}
